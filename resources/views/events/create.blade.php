    ...
    <form action="{{ route('events.store') }}" method="POST" enctype="multipart/form-data" id="eventForm">
        ...
        <div class="form-section">
                ...
                <div class="col">
                    ...
                    <select id="organizer_id" name="organizer_id" class="form-control" required>
                        ...
                        @foreach($organizers as $organizer)
                            <option value="{{ $organizer->id }}" data-src="{{ asset('storage/'.$organizer->profile) }}">{{ $organizer->name }}</option>
                        @endforeach
                    </select>
                </div>
            ...
            <div class="row mb-3">
                <div class="col">
                    ...
                    <select id="venue_id" name="venue_id" class="form-control" required onchange="if(this.value === 'addNew'){window.location.href='{{ url('venues/create') }}';}">
                        <option value="" disabled selected>Select Venue</option>
                        @foreach($venues as $venue)
                            <option value="{{ $venue->id }}" data-src="{{ asset('storage/'.$venue->profile) }}">{{ $venue->name }}</option>
                        @endforeach
                ...
                <div class="col">
                    <select name="category_ids[]" class="form-control" id="categoryDropdown" required multiple>
                        ...
                        @foreach($categories as $category)
                            <option value="{{ $category->id }}" data-icon="{{ asset('storage/' . $category->icon) }}">{{ $category->title }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
                ...
                <div class="col">
                    <select name="amenity[]" id="amenitySelect" class="form-control" required multiple>
                        ...
                        @foreach($amenities as $amenity)
                            <option value="{{ $amenity->id }}" data-icon="{{ asset('storage/' . $amenity->icon) }}">{{ $amenity->title }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
        ...
        <div class="form-section">
            ...
            <div class="row mb-3">
                ...
                <div class="col">
                    <input type="datetime-local" name="start_date" class="form-control" required min="{{ now()->format('Y-m-d\TH:i') }}">
                </div>
            </div>
            <div class="row mb-3">
        ...
        `;
            ...
            $('#addServiceProviderBtn').click(function() {
                ...
                @foreach ($serviceProviders as $serviceProvider)
                    optionsHtml +=
                        `<option value="{{ $serviceProvider->id }}" data-src="{{ asset('storage/' . $serviceProvider->profile) }}">{{ $serviceProvider->name }}</option>`;
                @endforeach
