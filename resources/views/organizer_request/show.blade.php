...
@section('content')
    ...
    <div class="container">
        ...
        <div class="detail-value">{{ $organizer->mobileUser->first_name }}  {{$organizer->mobileUser->last_name}}</div>

        <div class="detail-label">Organizer Name</div>
        <div class="detail-value">{{ $organizer->name }}</div>

        <div class="detail-label">BIO</div>
        <div class="detail-value">{{ $organizer->bio }}</div>

        <div class="detail-label">Work Location</div>
        <div class="detail-value">{{ $organizer->covering_area }}</div>

        @foreach($organizer->categories as $category)
            <div class="detail-label">Category</div>
            <div class="detail-value">{{ $category->title }}</div>
        @endforeach

        <div class="detail-label">Another Category</div>
        <div class="detail-value">{{ $organizer->other_category ?  : 'N/A' }}</div>

        <div class="detail-label">Profile</div>
        <div class="media-container">
            <img src="{{ asset('storage/' . $organizer->profile) }}" alt="Profile Image">
        </div>

        <div class="detail-label">Cover</div>
        <div class="media-container">
            <img src="{{ asset('storage/' . $organizer->cover) }}" alt="Cover Image">
        </div>
